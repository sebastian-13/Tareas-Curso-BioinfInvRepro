...
bioinformatica <- snpgdsOpen("Bioinformatica.gds")

# Hacer el PCA
pca<-snpgdsPCA(bioinformatica,autosome.only=FALSE, num.thread=2)
pc.percent <- pca$varprop*100
head(round(pc.percent, 2))

#Estimar los Eigenvalores
tab <- data.frame(sample.id = pca$sample.id,EV1 = pca$eigenvect[,1], # the first eigenvector
 EV2 = pca$eigenvect[,2], # the second eigenvector stringsAsFactors = FALSE)
head(tab)
